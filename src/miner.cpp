...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BAKERMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("BAKERMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("BAKERMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("baker-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("BAKERMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running BAKERMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadBAKERMiner() exception");
    } catch (...) {
        LogPrintf("ThreadBAKERMiner() exception");
    }

    LogPrintf("ThreadBAKERMiner exiting\n");
}
